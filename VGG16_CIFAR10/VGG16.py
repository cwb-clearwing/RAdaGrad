...
class VGG16(torch.nn.Module):
    def __init__(self, device='cpu'):
        ...
        self.layer = torch.nn.Sequential(
            ...
            Conv2d_lr(in_channels=64, out_channels=64, kernel_size=3, stride=1, padding=1, rank=64, r_min=10,
                      device=self.device),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            ...
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            Conv2d_lr(in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=1, rank=128, r_min=10,
                      device=self.device),
            nn.BatchNorm2d(128),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),
