        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                if hasattr(p,'is_matrix') and p.is_matrix:
                    temp = p.grad.detach().clone()
                    if pre:
                        state["pos_l"] = ((group["epsilon"]) * torch.eye(n=p.size()[0], device=p.device) + torch.diag(torch.diag(temp.matmul(temp.t()), 0))) ** (+1/4)
                        state["neg_l"] = torch.diag(torch.diag(state["pos_l"], 0) ** (-1))
                        state["pos_r"] = ((group["epsilon"]) * torch.eye(n=p.size()[1], device=p.device) + torch.diag(torch.diag(temp.t().matmul(temp), 0))) ** (+1/4)
                        state["neg_r"] = torch.diag(torch.diag(state["pos_r"], 0) ** (-1))
                        p.grad = state["neg_l"].matmul(p.grad.matmul(state["neg_r"]))
                    else:
                        state["pos_l"] = torch.eye(n=p.size()[0], device=p.device)
                        state["neg_l"] = state["pos_l"]
                        state["neg_r"] = torch.eye(n=p.size()[1], device=p.device)
            ...
            for p in group["params"]:
                ...
                if p.s == None:
                    ...
                    if hasattr(p,'is_matrix') and p.is_matrix: #and hasattr(p,'init') and not p.init:
                        ...
                        p.vh = vh
                        # 近似
                        p.data = u[:,0:p.r].matmul(torch.diag(s[0:p.r]).matmul(vh[0:p.r,:]))
                ...
                if hasattr(p,'is_matrix') and p.is_matrix:
                    if not pre:
                        state["pos_l"] = torch.eye(n=p.size()[0], device=p.device)
                        state["pos_r"] = torch.eye(n=p.size()[1], device=p.device)
                    grad1 = p.data.clone()
                    y1_temp = p.u[:,0:p.r].t()@state["pos_l"]
                    y1_part = y1_temp@p.u[:,0:p.r]@y1_temp@grad1
                    y1h = y1_part @ (torch.eye(n=p.size()[1], device=p.device) - p.vh[0:p.r,:].t()@p.vh[0:p.r,:])
                    y2_temp = state["pos_r"]@p.vh[0:p.r,:].t()
                    y2_part = grad1@y2_temp@p.vh[0:p.r,:]@y2_temp
                    y2 = (torch.eye(n=p.size()[0], device=p.device) - p.u[:,0:p.r]@p.u[:,0:p.r].t())@y2_part
                    # if integrator is None
                ...
                if hasattr(p,'is_matrix') and p.is_matrix:
                    ...
                    if rmax >= p.minimum_rank: 
                        ...
                        p.u[:,:rmax] = u[:,:rmax]
                        p.vh[:rmax,:] = vh[:rmax, :]
                        p.data = p.u[:,:rmax]@torch.diag(s[:rmax])@p.vh[:rmax,:]
                        p.r = rmax