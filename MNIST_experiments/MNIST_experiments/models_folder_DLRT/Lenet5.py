...
class Lenet5(torch.nn.Module):
    def __init__(self,device = 'cpu'):
        ...
        self.layer = torch.nn.Sequential(
            Conv2d_lr(in_channels = 1, out_channels = 20, kernel_size = 5, stride=1,rank = 20,device = self.device),  # 20 * 10 * 10 = 2000
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size = 2, stride=2),
            Conv2d_lr(in_channels = 20, out_channels = 50, kernel_size = 5, stride=1,rank = 50,device = self.device),  # 50 * 8 * 8 = 3200
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size = 2, stride=2), 
            torch.nn.Flatten(), # 800
            Linear(800,out_features = 500,rank = 500,device = self.device),  # 500
            torch.nn.ReLU(),
            Linear(500,out_features = 10, rank = 10, device = self.device)
        )
