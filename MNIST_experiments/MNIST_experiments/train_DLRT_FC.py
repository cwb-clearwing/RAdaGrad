...
if __name__ ==  '__main__':
  ...
  #from RGD_Optimizer.Riemannian_opt import RGD_Opt
  from optimizer_KLS_DLRT.train_custom_optimizer import *
  import tensorflow as tf
  from models_folder_DLRT.FC import FC
  ...
  parser.add_argument('--epochs', type=int, default=300, metavar='EPOCHS',
  ...
  parser.add_argument('--lr', type=float, default=0.07, metavar='LR',
  ...
  for index,theta in enumerate(thetas,0):
    ...
    for cv_run in range(args.cv_runs):
      ...
      #optimizer_RGD = RGD_Opt(f.parameters(),lr = args.lr,momentum=args.momentum,weight_decay = 0,epsilon = 0.01)
      path = './results_Full_Connection/_My_running_data_'+str(optimizer.theta)+'_'+str(current_optimizer)+'_'+str(cv_run)+'_batch_'+str(args.batch_size)+'_lr_'+str(args.lr)+'_epoch_'+str(MAX_EPOCHS)
