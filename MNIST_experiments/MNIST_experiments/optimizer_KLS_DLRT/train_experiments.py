...
def test_phase_metrics(NN, criterion, num_classes, dataloader,device = 'cpu', task = 'multiclass'):  # pseudo code
    
    NN.eval()
    # 2.1.2
    top1_metric = Accuracy(task = task, num_classes = num_classes).to(device)
    top5_metric = Accuracy(task = task, num_classes = num_classes, top_k = 5).to(device)
    # top-1 accuracy
    top_1_accuracy = 0.0
    top_5_accuracy = 0.0
    with torch.no_grad():
...
def train_and_finetune(NN,epochs,criterion,optimizer,scheduler,train_loader,validation_loader,path, num_classes, device = 'cpu',net_name = 'vgg',save_weights = False, task = 'multiclass'):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        acc_hist_test, top_5_acc_hist_test, loss_hist_test = test_phase_metrics(NN, criterion, num_classes, validation_loader, device)
...
def train_baseline(NN,epochs,criterion,optimizer,scheduler,train_loader,validation_loader,path, num_classes,device = 'cpu',net_name = 'vgg',save_weights = False, task = 'multiclass'):
